...
class CreditsState extends FlxState
{
    ...
    override public function create():Void
    {
        ...
        // Grupo de scroll com os créditos
        scrollGroup = new FlxGroup();
        add(scrollGroup);

        var startY:Float = FlxG.height + 20;
        ...
        for (i in 0...CREDITS.length)
        {
            ...
            // Cabeçalho de seção (sem nome associado)
            if (name == "" && role != "")
            {
            ...
            totalHeight = yPos + lineHeight - startY;
        }
    ...
    override public function update(elapsed:Float):Void
    {
        ...
        // Move todos os elementos do grupo
        for (item in scrollGroup.members)
        {
            if (item != null)
                item.y -= scrollSpeed * elapsed;
        }

        // Pisca o hint
        backHint.alpha = 0.4 + Math.sin(haxe.Timer.stamp() * 2) * 0.3;

        // Reinicia o scroll quando tudo passou
        if (scrollY >= totalHeight + FlxG.height)
        {
            scrollY = 0;
            for (item in scrollGroup.members)
            {
        ...
        {
            for (item in scrollGroup.members)
                if (item != null) item.y -= scrollSpeed * elapsed * 2;
        }
